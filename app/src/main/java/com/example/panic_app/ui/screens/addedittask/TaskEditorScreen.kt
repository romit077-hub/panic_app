    ...
    if (picker != null) {
        ...
        DisposableEffect(picker) {
            ...
            val dialog = if (picker == "date") {
                DatePickerDialog(context, { _, year, month, day -> choose(LocalDate.of(year, month + 1, day), current.toLocalTime()) },
                ...
                TimePickerDialog(context, { _, hour, minute -> choose(current.toLocalDate(), LocalTime.of(hour, minute)) },
...
private fun EditorField(label: String, value: String, onChange: (String) -> Unit,
    ...
    OutlinedTextField(value = value, onValueChange = onChange, label = { Text(label) },
        ...
        minLines = if (multiline) 3 else 1, isError = error != null,
